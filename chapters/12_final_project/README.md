...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .count-btn:hover, .count-btn.active { background: white; color: #667eea; transform: scale(1.05); }
        ...
        .option-btn:hover, .option-btn.selected { background: white; color: #667eea; }
        ...
        .answer-btn:hover:not(:disabled) { transform: translateY(-2px); box-shadow: 0 8px 20px rgba(102, 126, 234, 0.4); }
        .answer-btn:disabled { cursor: not-allowed; opacity: 0.7; }
        .answer-btn.correct { animation: correctFlash 0.5s ease-out; background: linear-gradient(135deg, #4caf50 0%, #45a049 100%); }
        .answer-btn.wrong   { animation: wrongShake 0.5s ease-out;   background: linear-gradient(135deg, #f44336 0%, #da190b 100%); }
        ...
        /* ── Results Screen ─────────────────────────────────── */
        .results-screen { background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; text-align: center; }
        ...
        .celebration-canvas { position: fixed; top: 0; left: 0; width: 100%; height: 100%; pointer-events: none; }
    ...
    <script>
        ...
        const utils = {
            ...
            calculatePoints(timeRemaining) {
                const percentage = Math.max(0, timeRemaining / 15);
                return Math.round(100 * percentage) + 50;
            }
        };
        ...
        const render = {
                            ...
                            <div class="color-select">
                                ${gameState.colors.map((color, idx) => `
                                    <div class="color-option ${idx === (player.colorIndex ?? colorIndex) ? 'selected' : ''}"
                                         ...
                                         data-player="${i}" data-color="${idx}"></div>
                                `).join('')}
                            </div>
                        </div>
                    `);
            ...
            attachSetupListeners() {
                document.querySelectorAll('.player-name-input').forEach(input => {
                    input.addEventListener('change', (e) => {
                        ...
                        gameState.players[idx].name = e.target.value || `Player ${idx + 1}`;
                ...
                document.querySelectorAll('.color-option').forEach(btn => {
                    btn.addEventListener('click', (e) => {
                        ...
                        gameState.players[playerIdx].colorIndex = parseInt(e.target.dataset.color);
                    });
                });
            },
            ...
            updateStartButtonState() {
                document.getElementById('startBtn').disabled =
                    gameState.playerCount === 0 || !gameState.players.every(p => p.name?.trim());
            },
            ...
            updateScoreboard() {
                document.getElementById('scoreBoard').innerHTML = gameState.players.map((player, idx) => `
            ...
            getBuzzKey(playerIdx) {
                return [['Q','W'], ['O','P'], ['U','I'], ['7','8']][playerIdx]?.join('/') || '?';
            },
            ...
            displayQuestion() {
                ...
                grid.innerHTML = answers.map(a => `<button class="answer-btn" data-answer="${a}">${a}</button>`).join('');
            ...
            handleAnswer(selectedAnswer, btn) {
                ...
                if (isCorrect) gameState.players[gameState.currentBuzzed].correct++;
            ...
            showScoreUpdate(playerIdx, points) {
                const scoreDiv = document.querySelectorAll('.score-card')[playerIdx].querySelector('.score');
            ...
            resultsScreen() {
                ...
                document.getElementById('leaderboard').innerHTML = sorted.map((player, idx) => {
                    const medal = ['🥇', '🥈', '🥉', ''][idx];
        ...
        const events = {
            ...
            async startGame() {
                ...
                gameState.questions = await utils.fetchQuestions(gameState.selectedRounds, gameState.selectedCategory, gameState.selectedDifficulty);
            ...
            startTimer() {
                ...
                const interval = setInterval(() => {
                    ...
                    timerCircle.classList.toggle('warning', timeRemaining <= 5 && timeRemaining > 2);
                    timerCircle.classList.toggle('danger', timeRemaining <= 2);
                }, 1000);
            },
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            document.querySelectorAll('.option-btn[data-difficulty]').forEach(btn => {
                btn.addEventListener('click', (e) => {
                    gameState.selectedDifficulty = e.target.dataset.difficulty;
                    render.updateButtonStates();
                });
            });
            document.querySelectorAll('.option-btn[data-rounds]').forEach(btn => {
                btn.addEventListener('click', (e) => {
                    gameState.selectedRounds = parseInt(e.target.dataset.rounds);
                    render.updateButtonStates();
                });
            });
